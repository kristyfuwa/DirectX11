...
bool Graphics::Initialize(int screenWidth, int screenHeight, HWND hWnd)
{
	...
	if (!m_pModel)
		return false;
	result = m_pModel->Initialize(m_pD3D->GetDevice(),L"tong.dds");
	if (!result)
	{
...
bool Graphics::Render(float dt)
{
	...
	result = m_pColorShader->Render(m_pD3D->GetDeviceContext(), m_pModel->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix,m_pModel->GetTexture());
	if (!result)
		return false;
